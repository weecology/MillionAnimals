...
def main():
    ...
    parser.add_argument(
        ...
        default=None,
        help=
        f'Specify a space-separated list of dataset names to download. If left unspecified, the script will download all of the official benchmark datasets. Available choices are {millionanimals.supported_datasets}.'
    )
    parser.add_argument(
        '--unlabeled',
        default=False,
    ...
    if config.datasets is None:
        config.datasets = millionanimals.benchmark_datasets

    for dataset in config.datasets:
        if dataset not in millionanimals.supported_datasets:
            raise ValueError(
                f'{dataset} not recognized; must be one of {millionanimals.supported_datasets}.'
            )

    print(f'Downloading the following datasets: {config.datasets}')
    for dataset in config.datasets:
        print(f'=== {dataset} ===')
        millionanimals.get_dataset(dataset=dataset,
