...
os.makedirs("/orange/ewhite/MillionAnimals/NEON_points/cropped_tiles", exist_ok=True)
...
for plot_id in filtered_points['plotID'].unique():
    ...
    #if not os.path.exists(f"/orange/ewhite/MillionAnimals/NEON_points/cropped_tiles/{plot_id}.tif"):
    try:
        cropped_tile = crop_raster(rgb_path=sensor_path, bounds=plot_points.total_bounds, savedir="/orange/ewhite/MillionAnimals/NEON_points/cropped_tiles", filename=plot_id)
    except:
    ...
    gdf["label"] = "Animal"
    gdf.root_dir = "/orange/ewhite/MillionAnimals/NEON_points/cropped_tiles"
    gdf["score"] = 1
    plot_results(gdf)

    annotations.append(gdf)

pd.concat(annotations).to_csv("/orange/ewhite/MillionAnimals/NEON_points/annotations.csv", index=False)
